...
`benchmark-file` parameters. If you want to use a custom docker/rkt container, execution options can be defined using the `benchmark-file`, such as `image`, `volumes`, `ports`, environment variables, execution arguments and so forth.
...
`nomi run --instancegroup-size=1 --dump-json --benchmark-file="./examples/benchmarkDefRkt.yaml"`

Run a benchmark that uses a custom fleet unit service's file:

`nomi run --dump-json --benchmark-file="./examples/benchmarkDef01.yaml" --unitfile-service=./examples/benchmarkDefUnitFile.service`

or

`nomi run --instancegroup-size=1 --dump-json --unitfile-service=./examples/benchmarkDefUnitFile.service --raw-instructions="(sleep 1) (start 200 100) (sleep 200) (stop-all)"`
